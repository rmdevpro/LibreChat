...
export function useMCPSelect({
  ...
  // Apply default-enabled servers when a new chat is initiated.
  // newChatGen is bumped by useNewConvo each time "New Chat" is clicked.
  // lastAppliedGenRef ensures defaults apply once per click, preserving
  // any manual changes the user makes within that chat session.
  useEffect(() => {
    ...
    if (configuredServers.size === 0) {
      return; // Servers not loaded yet — retry when they load
    }
    lastAppliedGenRef.current = newChatGen;
